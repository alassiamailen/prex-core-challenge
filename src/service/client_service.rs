...
pub trait ClientServiceTrait {
    ...
    /// Returns a [CommonError] if client_id has not existed or service throws any error
    async fn create_new_debit_transaction(&self, debit_transaction: NewDebitTransactionDto) -> Result<Decimal, CommonError>;
}
...
impl ClientServiceTrait for ClientService {
    ...
    /// Returns a [CommonError] if client_id has not existed or service throws any error
    async fn create_new_credit_transaction(&self, transaction: NewCreditTransactionDto) -> Result<Decimal, CommonError>{
    ...
    /// Returns a [CommonError] if client_id has not existed or service throws any error
    async fn create_new_debit_transaction(&self, transaction: NewDebitTransactionDto) -> Result<Decimal, CommonError>{
...
fn validate_client_document(&self,document_number: &str) -> Result<bool,CommonError>{
    ...
    match self.app_state.clients.read(){
        Ok(clients_map)=>{
            ...
            let validate_document = clients_map.values().any(|client|client.document_number == document_number);
        ...
        Err(_)=>{
            error!("validate_client_id - error -has occurred an error while try read in app_state");
            Err(CommonError::INTERNAL_SERVER_ERROR)
        }
    ...
    fn new_debit_on_client_account(&self, client_id: i32, debit_amount: Decimal) -> Result<Decimal, CommonError>{
